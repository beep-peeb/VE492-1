...
def consistent(assignment, csp, var, value):
    ...
    for constraint in constraints:
        if constraint.affects(var):
            variable = constraint.otherVariable(var)
            othervariable = assignment.assignedValues.get(variable)
            if not constraint.isSatisfied(value, othervariable):
                return False
    return True
...
"""
    ...
    orderValuesMethod: orderValues, leastConstrainingValuesH    constraints = csp.binaryConstraints
    for constraint in constraints:
        if constraint.affects(var):
            variable = constraint.otherVariable(var)
            othervariable = assignment.assignedValues.get(variable)
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
    ...
    for value in orderValuesMethod(assignment, csp, var):
        if consistent(assignment, csp, var, value):
            ...
            inferences = inferenceMethod(assignment, csp, value, var)
            if inferences is not None:
                for inference in inferences:
                    assignment.assignedValues[inference[0]] = inference[1]
        ...
        inferences = inferenceMethod(assignment, csp, value, var)
        for inference in inferences:
            assignment.assignedValues[inference[0]] = None
    return None
...
def minimumRemainingValuesHeuristic(assignment, csp):
    nextVar = None
    domains = assignment.varDomains
    char_value = assignment.assignedValues
    """Question 2"""
    set = []
    mini = float("inf")
    for values in domains:
        if char_value[values] is None:
            # print(len(domains[values]))
            # print(mini)
            # print(mini > len(domains[values]))
            # print(values)
            if (mini == len(domains[values])):
                set.append(values)
            if (mini > len(domains[values])):
                mini = len(domains[values])
                set = [values]
    # print(set)
    if len(set) == 1:
        return set[0]
    ...
    for item in set:
        tie = 0
        for constraint in constraints:
            other = constraint.otherVariable(item)
            if constraint.affects(item) and not assignment.isAssigned(other):
                tie = tie + 1
        # print(tie)
        if tie > max:
            max = tie
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
    """Question 3"""
    domains = assignment.varDomains
    val_list = orderValues(assignment, csp, var)
    dict = {}
    list1 = []
    list = []
    affectVar = []
    ...
    for constraint in constraints:
        if constraint.affects(var):
            ...
            affectVar.append(otherVariable)
    # print(orderValues(assignment,csp,val))

    # decide the number in the constraining domains
    for value in val_list:
        num = 0
        # print(domains)
        for affectvar in affectVar:
            if value in assignment.varDomains[affectvar]:
                num = num + 1
    ...
    # for val in domains:
    # print(orderValues(assignment,csp,val))
    # length = len(orderValues(assignment,csp,val))
    # if mini == length:
    # set.append(val)
    # if mini > length:
    # mini = length
...
def forwardChecking(assignment, csp, var, value):
    ...
    for constraint in constraints:
        if constraint.affects(var):
            # print(constraint.otherVariable(var))
            otherVariable = constraint.otherVariable(var)
            #print(otherVariable)
            if value in domains[otherVariable]:
                del domains[otherVariable]
                #print(domains[otherVariable])
                inferences.add((otherVariable, value))

    for values in assignment.assignedValues:
        if not domains[values]:
            for interfernce in inferences:
                domains[interfernce[0]].add(interfernce[1])
            return None"""
    constraints = csp.binaryConstraints
    # print(constraints)
    domains = assignment.assignedValues
    # print(domains)
    vardomain = assignment.varDomains
    # print(vardomain)
    for constraint in constraints:
        otherVariable = constraint.otherVariable(var)
        # print(type(otherVariable))
        if constraint.affects(var):
            # if domains != vardomain:
            if not assignment.isAssigned(otherVariable):  # not assigned variable
                valuedomain = vardomain[otherVariable]
                # print(valuedomain)
                othervariable = assignment.assignedValues.get(var)
                for values in valuedomain:
                    if not constraint.isSatisfied(value, values):
                        if len(vardomain[otherVariable]) <= 1:
                            return None
                        else:
                            inferences.add((otherVariable, values))

    for inference in inferences:
        # print(inferences)
        # print(vardomain[inference[0]])
        # print("\n")
        vardomain[inference[0]].remove(inference[1])
    return inferences
...
def revise(assignment, csp, var1, var2, constraint):
    inferences = set([])
    """Question 5"""
    var1Domain = assignment.varDomains[var1]
    var2Domain = assignment.varDomains[var2]
    num = 0
    # revise = True
    for value2 in var2Domain:
        revised = True
        for value1 in var1Domain:
            if constraint.isSatisfied(value1, value2):
                revised = False
        if revised:
            inferences.add((var2, value2))
            num = num + 1
    if len(inferences) == len(var2Domain):
        return None
    for inference in inferences:
    ...
    for constraint in constraints:
        if constraint.affects(var):
            ...
            if not assignment.isAssigned(othervariable):
                queue.append((othervariable, var))
    while len(queue) != 0:
        (i, j) = queue.pop()
        # print(i,j)
        temp = set([])
        for constraint in constraints:
            # temp = set([])
            if constraint.affects(i) and constraint.affects(j):
                temp = revise(assignment, csp, j, i, constraint)
        if temp is None:
            # print(temp)
            for tmp in inferences:
                # print(tmp)
                domains[tmp[0]].add(tmp[1])
            return None
        for tmp in temp:
            # print(tmp)
            inferences.add(tmp)
        # print(temp)
        if len(temp) != 0:
            for constraint in constraints:  # return to the queue
                if constraint.affects(i) and not constraint.affects(j):
                    otherVariable = constraint.otherVariable(i)
                    if not assignment.isAssigned(otherVariable):# not assigned
                        queue.append((otherVariable, i))
